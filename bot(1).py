...
log = logging.getLogger("btc-eth-bot")
...
def tg(text, group_text=None):
    ...
    for chat_id in CHAT_IDS:
        try:
            send_text = (
                group_text
                if group_text is not None and chat_id.startswith("-")
                else text
            )
...
def fetch(symbol):
    r = requests.get(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "User-Agent": "BTC-ETH-10m-Signal-Bot/4.0"
        },
        timeout=15
    )
...
def agg(mins):
    """
    Build CLOSED 10m candles from 1m candles.
    """
...
class Engine:
    ...
    def seed(self, closed):
        """
        Load existing history without creating
        historical signals.
        """
        ...
        if self.c:
            self.state["last_processed_10m"] = (
                next(reversed(self.c))
            )
        else:
            self.state["last_processed_10m"] = None
    ...
    def maybe_pre_alert(self, live10):
        """
        PRE-SIGNAL ONLY FOR LIVE CANDLE #8.

        #1 = Start
        #6 = Trigger
        #7 = same color as #6
        #8 = LIVE same color as #6

        Warning is sent during final ~2 minutes
        of live #8.

        If live #8 changes to Start color,
        warning is NOT sent.
        """
        ...
        elapsed = now - target_ts

        # Only final 2 minutes of candle #8.
        if elapsed < 480:
            return
        ...
        # Need #1 through #7 already closed.
        if sidx + 6 >= len(keys):
            return
        ...
        trigger_color = color(candle6)

        if trigger_color not in (
            "GREEN",
            "RED"
        ):
            return

        # #6 must be opposite to #1.
        if trigger_color == start_color:
            return

        # #7 must stay same color as #6.
        if color(candle7) != trigger_color:
            return

        # LIVE #8 must still have trigger color.
        if color(live10) != trigger_color:
            return
        ...
        log.info(
            "PRE-SIGNAL | %s | start=%s %s | #6=%s | #7=%s | LIVE #8=%s %s",
            self.symbol,
            utc(start_ts),
            start_color,
            trigger_color,
        ...
        group_text = (
            ...
            "Timeframe: 10m\n"
            f"Очікується: {direction}\n\n"
            "⚠️ Сигнал буде тільки після "
            "повного закриття свічки #8."
        )
    ...
    def evaluate(self, ts):
        ...
        for p in self.pending:
            ...
            if rel in (1, 2):
                ...
                if color(cur) != trigger_color:
                    ...
                    log.info(
                        "CANCEL SETUP | %s | start=%s | "
                        "candle #%d changed color",
                        self.symbol,
                        utc(p["start_ts"]),
                        rel + 6
                    )
            ...
            # RESULT #9 - #15
            #
            # ANY candle with Start color = WIN
            # ========================================================
            ...
            if (
                rel <= 9
                and color(cur) == start_color
            ):

                log.info(
                    "RESULT WIN | %s | start=%s | result candle=%d",
                    self.symbol,
                    utc(p["start_ts"]),
                    rel + 6
                )
            ...
            # ========================================================
            # #15 CLOSED WITHOUT WIN
            # ========================================================

            if rel >= 9:

                log.info(
                    "RESULT LOSS | %s | start=%s | "
        ...
        # #1 must be last candle of same-color run.
        if sidx + 1 < len(keys):
...
# ================================================================
# CREATE SEPARATE ENGINE FOR EACH SYMBOL
# ================================================================
...
for symbol in SYMBOLS:
    ...
    state_file = os.getenv(
        f"STATE_FILE_{symbol}",
        f"state_{symbol}.json"
    )
...
def main():
    ...
    log.info(
        "Started BTCUSDT + ETHUSDT 10m signal bot v4-fixed (REST polling)"
    )

    log.info(
        "Config: poll=%ss, chats=%d, token_configured=%s",
        POLL,
        len(CHAT_IDS),
        bool(TOKEN)
    )
    ...
    log.info(
        "Rule: #1 Start | #6 trigger | "
        "#7-#8 same color as #6 | #9-#15 result"
    )
    ...
    if TOKEN and CHAT_IDS:
        ...
        tg(
            ...
            "This test confirms Telegram delivery."
        )
    ...
    while True:
        ...
        for symbol, engine in engines.items():
            ...
            try:
                ...
                # ------------------------------------------------
                # LIVE #8 PRE-SIGNAL CHECK
                # ------------------------------------------------

                live10 = current_10m(mins)

                if live10:
                    engine.maybe_pre_alert(
                        live10
                    )
